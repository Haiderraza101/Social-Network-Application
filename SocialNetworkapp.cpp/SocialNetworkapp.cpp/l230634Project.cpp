...
public:
	...
	void viewrecentpageposts()
	{

		for (int i = 0; i < index; i++)
		{
			if (((date::currentday == timeline[i]->getday()) || date::currentday == timeline[i]->getday() + 1 ||
				date::currentday == timeline[i]->getday() + 2 || date::currentday == timeline[i]->getday() + 3) &&
				(date::currentmonth == timeline[i]->getmonth()) && (date::currentyear == timeline[i]->getyear()))
			{
				cout << title;
	...
	void viewtimeline()
	{
		cout << "\n--------------------------------------------------------------------------------------------------------------" << endl;
		cout << title << endl;
		for (int i = 1; timeline[i] != nullptr; i++)
		{
			cout << title;
			timeline[i]->printactivity();
			timeline[i]->displaydate();
...
public:
	...
	void printmemoryinfo(memory* m) {
		if (m->getsharedby() != nullptr)
		{
			m->getsharedby()->printname();
		}
	...
	void printregularposts() {
		for (int i = 3; timeline[i] != nullptr; i++) {
	...
	void viewhomepage()
	{
		...
		for (int i = 0; i < friendcount; i++)
		{
			if (friendlist[i] != nullptr)
				friendlist[i]->viewrecentuserposts();
		}

		for (int i = 2; i < pagecount; i++)
		{
			if (pagelist[i] != nullptr)
				pagelist[i]->viewrecentpageposts();
		}
	}
	posts** gettimeline()
	{
...
public:
	...
	void run() {
		...
		if (currentuser != nullptr) {
			...
			cout << "-------------------------------------------------------------------------------------------------------------" << endl;
			const char* postid2 = "post10";
			posts* ptr2 = searchpostbyid(postid2);
			int viewpost3 = 9;
			...
			currentuser->printmemoryinfo(memoryptr);
			currentuser->viewtimeline();
